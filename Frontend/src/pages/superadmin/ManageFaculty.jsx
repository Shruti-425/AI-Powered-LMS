...
function ManageFaculty() {
  ...
  const loadUsers = () =>
    getUsers({ role: "instructor", ...(search ? { search } : {}) }).then(setUsers);
  ...
  const startEdit = (user) => {
    ...
    setForm({ name: user.name, email: user.email, password: "", role: "instructor" });
  };
  ...
  return (
    <DashboardLayout>
      <h1 className="text-3xl font-bold mb-2">Manage Faculty</h1>
      <p className="text-slate-500 mb-6">Add, edit, and remove instructors</p>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!loading && users.length === 0 && (
              <tr>
                <td colSpan={4} className="p-4 text-gray-500">No faculty found.</td>
              </tr>
            )}
            {users.map((user) => (
              <tr key={user.user_id} className="border-t">
                ...
                <td className="p-4">{user.courses_taught}</td>
